...
async function createLivroInfo(livroInfo) {
    ...
    try { 
        await info.connect();
        return await info.db("bookStore").collection("livroInfo").insertOne(livroInfo);
    } catch (err) {
        throw err;
    } finally {
        await info.close();
...
async function updateLivroInfo(livroInfo) {
    ...
    try {
        await info.connect();
        return await info.db("bookStore").collection("livroInfo").updateOne(
...
async function deleteLivroInfo(livroId) {
    ...
    try { 
        await info.connect();
        return await info.db("bookStore").collection("livroInfo").deleteOne({livroId});
    } catch (err) {
        throw err;
    } finally {
        await info.close();
...
async function getLivroInfos(livroId) {
    ...
    try {
        await info.connect();
        return await info.db("bookStore").collection("livroInfo").findOne({ livroId });
    } catch (err) {
        throw err;
    } finally {
        await info.close();
